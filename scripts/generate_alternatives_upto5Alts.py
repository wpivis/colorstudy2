"""
This script generates alternatives_upto5.json by calling a Colorgorical endpoint in batches and applying LOCAL sectoring (ΔH=20, ΔR=10) to filter candidates. It keeps the top-5 survivors per palette-index. It is the same script as generate_alternatives.py from Study-1 and generate_alternatives_2.py from this Study-2.
"""
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Generate alternatives_upto5.json using LOCAL sectoring (ΔH=20, ΔR=10) and keep top-5 survivors."
    )
    parser.add_argument("--palettes", default="../palettes.json", help="Path to palettes.json")
    parser.add_argument("--out", default="alternatives_upto5.json", help="Output JSON path")
    parser.add_argument(
        "--colorgorical-url",
    ...
    parser.add_argument("--topk", type=int, default=5, help="Alternatives to keep per palette-index (default 5)")
    ...
    for palette in palettes:
        ...
        for selected_idx in [0, 1, 2]:
            ...
            # We want to have minimum 2 survivors, but anything up to 5 is fine
            MIN_SURVIVORS = 2
            ...
            if len(survivors) < args.topk:
                print(f"Only got {len(survivors)}/{args.topk} survivors for palette {palette_id} idx {selected_idx} — keeping what we have", flush=True)
