...
server.registerTool("validateglobalconfig", {
    ...
    try {
        ...
        const filePath = path_1.default.join(__dirname, '..', '..', 'public', 'global.json');
        let fileContent;
        try {
        ...
        const validationResult = (0, utils_1.validateGlobalConfig)(data);
        if (validationResult.isValid) {
            ...
            const errorMessages = validationResult.errors.map(error => {
                ...
                if (error.instancePath && error.message) {
                    return `You have an error at ${error.instancePath}: ${error.message}${((_a = error.params) === null || _a === void 0 ? void 0 : _a.action) ? ` - ${JSON.stringify(error.params)}` : ''}`;
                }
...
server.registerTool("validatestudyconfig", {
    ...
    try {
        ...
        const resolvedPath = path_1.default.isAbsolute(filePath) ? filePath : path_1.default.join(__dirname, '..', '..', filePath);
        let fileContent;
        try {
        ...
        const validationResult = (0, utils_1.validateStudyConfig)(data);
        if (validationResult.isValid) {
