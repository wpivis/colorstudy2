...
import {
  IconAdjustmentsHorizontal, IconBubbleText, IconChevronDown, IconCodePlus, IconDots, IconHtml, IconDragDrop, IconNumber123, IconRadio, IconSelect, IconSquares,
...
export function ResponseVisualization({
  ...
  const vegaLiteSpec = useMemo(() => {
    ...
    if (response.type === 'radio' || response.type === 'dropdown' || response.type === 'checkbox' || response.type === 'ranking-sublist' || response.type === 'ranking-categorical' || response.type === 'ranking-pairwise') {
      const spec = {
        ...baseSpec,
        data: { values: questionData },
        mark: 'bar',
    ...
    <Paper p="lg" withBorder ref={ref}>
      ...
      <Collapse in={opened} mah={400}>
        ...
        <SimpleGrid cols={2} h={360}>
          <ScrollArea mih={200}>
            {(response.type !== 'metadata' && response.type !== 'shortText' && response.type !== 'longText' && response.type !== 'reactive' && response.type !== 'ranking-sublist' && response.type !== 'ranking-categorical' && response.type !== 'ranking-pairwise') ? (
              <VegaLite
